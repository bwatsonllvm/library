...
const CATEGORY_META = {
  ...
  'mlir-workshop':  { label: 'MLIR Workshop',   order: 10 },
  'workshop':       { label: 'Workshop',        order: 11 },
...
function renderTalkTypeFilterChip(category, count) {
  const normalizedCategory = String(category || 'other').trim().toLowerCase() || 'other';
  const label = categoryLabel(normalizedCategory);
  const badgeKey = normalizedCategory.toLowerCase().replace(/[^a-z0-9-]/g, '-') || 'other';
  return `
...
const _SVG_SLIDES = `<svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="4" y="5" width="14" height="10" rx="1.8"/><path d="M7 9h8"/><path d="M7 12h5"/><path d="M9 19h11a1 1 0 0 0 1-1V9a1 1 0 0 0-1-1"/></svg>`;
const _SVG_POSTER = `<svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="6" y="3" width="12" height="14" rx="1.5"/><path d="M9 7h6"/><path d="M9 10h6"/><path d="M12 17v4"/><path d="M9.5 21h5"/></svg>`;
...
function placeholderSvgForCategory(category) {
  return { 'mlir-workshop': _SVG_TOOL, workshop: _SVG_TOOL, panel: _SVG_CHAT, bof: _SVG_CHAT, 'open-design-meeting': _SVG_CHAT }[category] ?? _SVG_DOC;
}
...
function mediaPlaceholderForTalk(talk) {
  ...
  const category = String(talk && talk.category || '').trim().toLowerCase();

  if (!videoHref) {
    if (category === 'poster' || posterHref) {
      return { svg: _SVG_POSTER, label: 'Poster' };
    }
    if (slidesHref) {
      return { svg: _SVG_SLIDES, label: 'Slides' };
    }
  }

  if (isAppleDeveloperVideoUrl(videoHref)) {
    return { svg: _SVG_TV, label: 'Video' };
  }
...
function renderCard(talk, tokens) {
  ...
  const posterHref = sanitizeExternalUrl(talk.posterUrl);
  const slidesHref = sanitizeExternalUrl(talk.slidesUrl) || posterHref;
  ...
  const videoLinkHtml = videoHref
    ? `<a href="${escapeHtml(videoHref)}" class="card-link-btn card-link-btn--video" target="_blank" rel="noopener noreferrer" aria-label="${escapeHtml(videoMeta.ariaLabel)}">
        ${videoIcon}
  ...
  const slidesLinkHtml = slidesHref
    ? `<a href="${escapeHtml(slidesHref)}" class="card-link-btn" target="_blank" rel="noopener noreferrer" aria-label="View ${escapeHtml(String(talk.category || '').trim().toLowerCase() === 'poster' || posterHref ? 'poster' : 'slides')}: ${titleEsc} (opens in new tab)">
        ...
        <span aria-hidden="true">${escapeHtml(String(talk.category || '').trim().toLowerCase() === 'poster' || posterHref ? 'Poster' : 'Slides')}</span>
      </a>`
    : '';

  const githubLinkHtml = githubHref
    ? `<a href="${escapeHtml(githubHref)}" class="card-link-btn" target="_blank" rel="noopener noreferrer" aria-label="GitHub repository: ${titleEsc} (opens in new tab)">
...
function renderActiveFilters() {
  ...
  if (state.speaker) {
    pills.push(createActiveFilterPill(
      ...
      removeSpeakerFilter,
      {
        ...
        workAriaLabel: `See All Work for speaker ${state.speaker}`,
      }
    ));
  }
  ...
  if (state.query) {
    ...
    if (state.activeSpeaker && state.activeSpeaker.toLowerCase() === state.query.toLowerCase()) {
      ...
      workAriaLabel = `See All Work for speaker ${state.query}`;
    } else if (state.activeTag && state.activeTag.toLowerCase() === state.query.toLowerCase()) {
      typeLabel = 'Key Topic';
      workHref = buildAllWorkUrl('topic', state.query);
      workAriaLabel = `See All Work for key topic ${state.query}`;
    }
    pills.push(createActiveFilterPill(
      typeLabel,
      state.query,
...
function initSearch() {
  ...
  if (useUniversalSearch) {
    ...
    searchForm.addEventListener('submit', (event) => {
      const submitType = normalizeValue(searchForm.dataset.searchSubmitType || 'query');
